...
- feat: `/vault-cli:plan-task` Step 5's E2E verify subtask check now also rejects *vague* verify subtasks, not just missing ones. The verify subtask body must describe HOW verification happens (a concrete procedure, observable, or artifact check) — bare promises like *"Verify the endpoint"* fail; concrete steps like *"curl /widgets, confirm 200 + body matches schema"* pass. LLM judgment via the same vagueness pattern the existing **Subtasks reach the goal** gate uses — no verb list, no regex. Closes the *vague-verify* hole that PR #15's *missing-verify* fix left open: a verify subtask that doesn't say *how* lets the owner tick the box without doing real work, same dishonest-tick outcome.
