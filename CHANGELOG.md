...
- work-on-task-assistant: connect the task to a **fleet-spawned** session. The session-connect title match was an exact string compare, but a supervisor prefixes the title it gives a spawned session (`⚙ <task>`) at spawn — before the step runs — so the compare returned zero and the guard took its "refusing to guess" branch, leaving `claude_session_id` and `metrics_sessions` both empty. Measured 2026-09-19 on the Personal vault: exact match 0 sessions, decoration-stripped 1; three tasks spawned through the documented fresh-start shape that afternoon carried 0 session-id fields each, against 2 for a task spawned the older way. The guard was correct and the matcher was wrong. The match is now **strip + scope + require uniqueness**: strip the leading decoration, restrict the scan to **live** transcripts (`LIVE_WINDOW` = 5 min, per `docs/session-liveness.md`), and still refuse on zero or multiple. Stripping alone is not sufficient — across a real fleet 81 stripped titles collide, and the scan carried no recency filter, so a strip-only fix would also bind tasks to sessions that had already ended.
