...
- docs: record at `agents/task-creator.md` step 11b that the step is to be **removed rather than repaired**, with the measurement behind it. Of 111 non-terminal task→goal declarations missing from their goal's `# Tasks` list in the Personal vault, 56 predate the step, 21 postdate it, and 32 were attached to an existing task *after* creation — a path on which no code runs at all: `markGoalCheckbox`, the only writer of a goal's `# Tasks` list, is called from exactly one place, `pkg/ops/complete.go:158`, so an Obsidian hand edit or a `vault-cli task add goals` can never reach step 11b. Backfilling and repairing the step both leave that class producing. The decision is to demote `# Tasks` to a derived view of `goals:` frontmatter; the implementation lands separately.
