...
- feat: add `scripts/goal-task-link-count.sh`, the re-runnable form of the 2026-09-19 measurement that 112 of 256 non-terminal task→goal declarations in the Personal vault had no matching entry in the goal's `# Tasks` list. Re-derived independently the same day it gives **275 declarations / 125 missing (45.5%)**, of which 14 name a goal file that does not exist. The figure is rule-sensitive — excluding those 14 gives 111 — so the script enumerates its rules instead of leaving them implied: terminal tasks are excluded from the denominator, a declaration whose goal file is absent counts as missing but is subtotalled separately, `|alias` / `#heading` / `dir/` wikilink forms resolve to one title, and membership is the **union** of every `# Tasks` section in a goal file. That last rule is not hypothetical: `Automate General Alerts Monitoring` carries two `# Tasks` headings, and the obvious first-heading-only implementation moves 2 declarations from linked to missing. It was found by running against the vault, not by reading the code — as were the two bugs the script shipped and fixed during authoring (goal existence derived from the link stream, so a goal with an empty `# Tasks` section read as absent; and the membership stream emitted after the declarations that test against it, which reported every declaration missing — the exact shape of the defect being measured, failing silently green). Tested against a fixture whose answer differs from the real vault's, plus a mutation case that proves the numbers are derived rather than recited.
