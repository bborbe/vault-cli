...
- fix: `session-close` Phase 3.5 now detects **detached-HEAD worktrees**, which the branch-based orphan tests cannot classify. `git branch --show-current` is empty for a detached worktree, so the `ls-remote` probe matches nothing, test (b)'s merge-commit name comparison never fires, and test (a)'s `rev-list --count` reads 0 because a release tag is an ancestor of master — three negatives that the phase's own rule reads as "leave it alone". Detached worktrees are not rare: the build-at-a-pin pattern (`git worktree add /tmp/<svc>-<tag> <tag>`) is what operational runbooks prescribe for rebuilding a component without moving its pin, and one fleet rebuild creates a dozen. Observed 2026-09-13: 15 such worktrees across 11 repos, every one invisible to the check; they were found only by listing worktrees by hand. Now flagged outright, skipping (a)/(b) — a detached worktree holds no branch and therefore no unpushed work, so removal cannot lose anything. Text-only: one command file.
