...
2. If it is **empty or absent**: detect the current session's UUID by title-match, never by newest-transcript. The `ls -t ... | head -1` mtime scan is forbidden — in a fleet of concurrent sessions the newest transcript is almost never the current session (observed: a fresh headless Start session got bound to a live unrelated session this way). Match the goal's name against each transcript's CURRENT title (the `customTitle` of the LAST `custom-title` line — transcripts are append-only, so last is newest; ignore `custom-title` lines with no `customTitle` key):
   ```bash
