...
- With open entries, re-surface them under the heading `📋 Open with the operator`, one line per entry — `kind · what · state · age` — exactly as the manager's own round renders it. Do not re-word them: an entry's `text` is the operator's own wording, and paraphrasing it across a compaction is how the ask drifts.
