...
**Close-out fields (`aborted`):** vault-cli enforces that an `aborted` close-out records a disposition. Two frontmatter fields carry it (both may still be recorded on `completed`, but are never required there):

- `aborted_reason` — free text explaining the close-out.
...
The CLI rejects `aborted` (via `task set` / `goal set` status aborted) unless both fields are present; the error names the missing fields, asks what the task owns (trigger / gate / threshold / recurring check) and where it moves. Values are user-supplied strings persisted through the YAML serializer. Pre-existing reason-less close-outs are not backfilled. `task set`, `task complete`, `goal set`, and `goal complete` accept --reason <text> and --gate-successor <successor|none> to record both fields in the same invocation. `completed` never consults the close-out fields: `task complete`, `goal complete`, `task set <name> status completed`, `goal set <name> status completed`, and the `task update` checkbox sync all succeed without them (spec 039).
