...
`blocked_by` declares the tasks this task cannot start before. It is a YAML list of task names; each entry may be a plain name (`Blocker Task`) or a wikilink (`[[Blocker Task]]`) — both forms resolve the same way. A scalar value (`blocked_by: Blocker Task`) is malformed and reads as an empty list, so it never blocks.

Names match task files in the tasks directory, ignoring case and stripping the `[[` `]]` brackets. Resolution is same-kind only: a task's blockers are tasks, never goals, and the lookup is exact-name — a substring is not a match.

The task is blocked while at least one named blocker is not `status: completed`. A blocker whose file is missing, unreadable, or carries no parseable status counts as **not** completed: the safe default is "cannot verify it is done, so do not start". Only a single status read is performed per blocker — a blocker's own `blocked_by` is never followed, so a dependency cycle leaves both tasks blocked and terminates immediately rather than hanging.
