...
var _ = Describe("ObjectiveFrontmatter", func() {
	var (
		ctx context.Context
		fm  domain.ObjectiveFrontmatter
	)

	BeforeEach(func() {
		ctx = context.Background()
		fm = domain.NewObjectiveFrontmatter(nil)
	})
	...
	Describe("StartDate", func() {
		...
		It("parses a YAML date literal (time.Time path)", func() {
			fm = domain.NewObjectiveFrontmatter(
		...
		It("parses a string value (hand-authored date)", func() {
			fm = domain.NewObjectiveFrontmatter(map[string]any{"start_date": "2025-01-15"})
			result := fm.StartDate()
			Expect(result).NotTo(BeNil())
			Expect(result.Time().UTC().Format("2006-01-02")).To(Equal("2025-01-15"))
		})
	...
	Describe("TargetDate", func() {
		...
		It("parses a string value (hand-authored date)", func() {
			fm = domain.NewObjectiveFrontmatter(map[string]any{"target_date": "2025-12-31"})
			result := fm.TargetDate()
			Expect(result).NotTo(BeNil())
			Expect(result.Time().UTC().Format("2006-01-02")).To(Equal("2025-12-31"))
		})
