...
var _ = Describe("TaskStatus", func() {
	...
	Describe("UnmarshalYAML", func() {
		...
		Context("canonical values", func() {
			It("unmarshals todo", func() {
				yamlData := []byte("status: todo")
			...
			It("unmarshals in_progress", func() {
				yamlData := []byte("status: in_progress")
				var result TestStruct
				err := yaml.Unmarshal(yamlData, &result)
				Expect(err).To(BeNil())
				Expect(result.Status).To(Equal(domain.TaskStatusInProgress))
			})

			It("unmarshals completed", func() {
				yamlData := []byte("status: completed")
				var result TestStruct
				err := yaml.Unmarshal(yamlData, &result)
				Expect(err).To(BeNil())
				Expect(result.Status).To(Equal(domain.TaskStatusCompleted))
			})

			It("unmarshals backlog", func() {
				yamlData := []byte("status: backlog")
				var result TestStruct
				err := yaml.Unmarshal(yamlData, &result)
				Expect(err).To(BeNil())
				Expect(result.Status).To(Equal(domain.TaskStatusBacklog))
			})

			It("unmarshals hold", func() {
				yamlData := []byte("status: hold")
				var result TestStruct
				err := yaml.Unmarshal(yamlData, &result)
				Expect(err).To(BeNil())
				Expect(result.Status).To(Equal(domain.TaskStatusHold))
			})

			It("unmarshals aborted", func() {
				yamlData := []byte("status: aborted")
				var result TestStruct
				err := yaml.Unmarshal(yamlData, &result)
				Expect(err).To(BeNil())
				Expect(result.Status).To(Equal(domain.TaskStatusAborted))
			})
		})

		Context("alias values", func() {
			It("unmarshals done as completed", func() {
				yamlData := []byte("status: done")
				var result TestStruct
				err := yaml.Unmarshal(yamlData, &result)
				Expect(err).To(BeNil())
				Expect(result.Status).To(Equal(domain.TaskStatusCompleted))
			...
			It("unmarshals current as in_progress", func() {
				yamlData := []byte("status: current")
				var result TestStruct
				err := yaml.Unmarshal(yamlData, &result)
				Expect(err).To(BeNil())
				Expect(result.Status).To(Equal(domain.TaskStatusInProgress))
			...
			It("unmarshals next as todo", func() {
				yamlData := []byte("status: next")
				var result TestStruct
				err := yaml.Unmarshal(yamlData, &result)
				Expect(err).To(BeNil())
				Expect(result.Status).To(Equal(domain.TaskStatusTodo))
			...
			It("unmarshals deferred as hold", func() {
				yamlData := []byte("status: deferred")
				var result TestStruct
				err := yaml.Unmarshal(yamlData, &result)
				Expect(err).To(BeNil())
				Expect(result.Status).To(Equal(domain.TaskStatusHold))
		...
		Context("invalid values", func() {
			It("returns error for invalid status", func() {
				yamlData := []byte("status: garbage")
				var result TestStruct
				err := yaml.Unmarshal(yamlData, &result)
				Expect(err).NotTo(BeNil())
				Expect(err.Error()).To(ContainSubstring("invalid task status"))
				Expect(err.Error()).To(ContainSubstring("garbage"))
			})

			It("returns error for empty status", func() {
				yamlData := []byte("status: ''")
				var result TestStruct
				err := yaml.Unmarshal(yamlData, &result)
				Expect(err).NotTo(BeNil())
				Expect(err.Error()).To(ContainSubstring("invalid task status"))
		...
		Context("marshaling back to YAML", func() {
			It("always writes canonical value", func() {
				// Create task with status set to completed
				task := TestStruct{Status: domain.TaskStatusCompleted}
				yamlData, err := yaml.Marshal(&task)
				Expect(err).To(BeNil())
				Expect(string(yamlData)).To(Equal("status: completed\n"))
			})

			It("writes todo as todo", func() {
				task := TestStruct{Status: domain.TaskStatusTodo}
				yamlData, err := yaml.Marshal(&task)
				Expect(err).To(BeNil())
				Expect(string(yamlData)).To(Equal("status: todo\n"))
			})
		})
	})
})
