...
- Confirms the interactive terminal branch, its 5-minute cap, and the resume scenario are unchanged, and replaces the spec's git-based "scenario 005 untouched" guard with a content pin, because git is masked in this container and a git command here would report a false pass.
...
- Flags spec evidence greps that can never match the real source: several carry a literal double quote the source does not contain, and the spec's error-string list for the shared validation helper is pre-spec-045 and must NOT be applied.
...
- `docs/work-on-session-lifecycle.md` — the durable design record this implementation realizes. Its task-path sections were rewritten by a later reversion; fixing that is prompt 3's job — do NOT edit the doc here.
...
12. **Confirm the detachment integration test.** `pkg/ops/claude_session_detach_test.go` must contain the spec "child outlives a cancelled parent wait" that writes a real shell script (`#!/bin/sh\nsleep 6\ntouch <sentinel>`), cancels the context after 500ms, asserts `StartSession` returns an error, asserts the sentinel does NOT exist yet, and then `Eventually(..., "20s", "200ms")` asserts the sentinel appears — proving the detached child survived the parent's cancelled wait. It constructs the starter with `ops.NewClaudeSessionStarter(script, ops.NewSessionLockerWithDir(lockDir))` (the locker is spec 042's; keep it). If the file or spec is missing, report `"status":"failed"` — do not re-implement from the spec, whose snippet uses a 12s script and a 1s cancel, neither of which matters to the invariant.

13. **Confirm the AC10 guards by reading, then by grep — WITHOUT git.** git is masked here (see `<context>`), so the spec's `git diff --exit-code HEAD` guard is replaced by a content pin:
    - `scenarios/005-work-on-resume-auto-invokes-subtask.md` must be byte-identical to today: its sha256 is `973840d5a8c6a55cb84c6db10c9c24ab2ff269b1ba0fa82e31ab1ac630ea0153` and it is 87 lines. If either differs, you modified a file this prompt must not touch — revert that edit; do NOT update the expected values.
    - `defaultCommandRunner` is defined once and referenced by both constructors — the grep count in `pkg/ops/claude_session.go` must be exactly 3.
    - `context.WithTimeout` must appear exactly once, on the interactive branch.
    - `mocks/claude-session-starter.go` must be untouched: `ClaudeSessionStarter.StartSession`'s signature is `StartSession(context.Context, string, string, string, string, bool) error` — six parameters, unchanged. Verify with the signature grep in `<verification>`; do NOT pin this file's hash, because `make generate` rewrites it (`rm -rf mocks` + `go generate`).

14. **Self-check before finishing.** Re-read the two changed hunks and walk spec 041 ACs 1-6 and 10 against them, stating which artifact satisfies each. Run every command in `<verification>` and confirm each holds. The spec evidence greps flagged in `<verification>` as quoting artifacts must NOT be "fixed" by editing source strings.
...
- Do NOT alter error strings to satisfy a grep pattern. Several of the spec's evidence greps carry a literal double quote the source does not contain (see `<verification>`); the source strings are correct as written.
...
SECONDARY — AC10 content pin (git-free substitute for `git diff --exit-code HEAD`, because git is masked here):
```
sha256sum scenarios/005-work-on-resume-auto-invokes-subtask.md   # must print 973840d5a8c6a55cb84c6db10c9c24ab2ff269b1ba0fa82e31ab1ac630ea0153
wc -l < scenarios/005-work-on-resume-auto-invokes-subtask.md     # must print 87
grep -c 'func (fake \*ClaudeSessionStarter) StartSession(arg1 context.Context, arg2 string, arg3 string, arg4 string, arg5 string, arg6 bool) error' mocks/claude-session-starter.go   # == 1
```
...
NOTE on the AC10 guard the spec lists as container-executable: `git diff --exit-code HEAD -- scenarios/005-work-on-resume-auto-invokes-subtask.md` CANNOT run in this container. The sha256 + line-count pin above is its substitute. The git form belongs on the spec's operator rung.
</verification>
