...
- IMPORTANT FLAG FOR THE HUMAN REVIEWER: the task-side half of this spec was deliberately REVERTED in the tree after the spec was approved, because persisting only after the turn left the field empty while the child ran and the child's own session-connect then bound the task to a live, unrelated session. This prompt implements the spec as approved and re-applies the reversion's opposite; the reviewer must adjudicate at audit time. Two later releases (v0.131.2 and v0.138.1) hardened exactly the mechanism the reversion was defending against, which is recorded in the reviewer comment — read it before deciding.
- The docs/changelog prompt is coupled to that decision: if this prompt is rejected at audit, prompt 3 must be rejected too.
</summary>

<objective>
Make the task path of `work-on` persist `claude_session_id` only after the detached headless turn has finished cleanly, so no failure path can leave a resumable-looking id behind — matching the goal path, which already ships this behaviour. Covers spec 041 ACs 7-9 and depends on prompt 1 having shipped (its block-until-exit `StartSession` is what these tests exercise).
</objective>
...
- `pkg/ops/goal_workon.go` — the whole file (240 lines). This is the structural TEMPLATE the reordered task path must match: its `handleClaudeSession` starts the turn first and persists only after it returns cleanly, on both the interactive and non-interactive branches, with no compensating clear.
- `pkg/ops/topic_workon.go` — the whole file. The THIRD work-on operation, and the second structural TEMPLATE: its `handleClaudeSession` is already start-then-persist on both branches with no compensating clear, exactly like the goal path. Read it so the reordered task path matches both siblings, not just one. Do not modify it.
...
- `pkg/ops/goal_workon_test.go` — read `Context("when persisting the goal session id after the child exits", ...)` and the context that proves a failed goal turn persists nothing, to confirm the goal path is already in the target state. Do not modify this file.
...
NOTE: `.dark-factory.yaml` is `workflow: direct` with no `hideGit: true`, so git *should* be available — but this prompt has no git commands; the AC10 `scenarios/005` guard was verified in prompt 1.
</context>
...
- Commit 247a789 (released v0.117.1) applied spec 041 to workon.go: persist claude_session_id only AFTER the turn, no compensating clear. Its CHANGELOG bullet reads: "non-interactive `task work-on` / `goal work-on` now wait for the detached headless turn to finish before persisting `claude_session_id` ... The wait is bounded by a 30m turn timeout ... the interactive TTY branch is unchanged."
...
- The reversion did NOT touch claude_session.go's block-until-exit + validation (spec 041 prompt 1), the spec 042 per-session lock, or goal_workon.go (which still persists post-exit). `topic_workon.go` did not exist at the time and was written later with the goal path's post-exit shape, so it too is unaffected.
...
NEW EVIDENCE SINCE THIS PROMPT WAS FIRST DRAFTED — TWO LATER RELEASES HARDENED EXACTLY THE MECHANISM THE REVERSION WAS DEFENDING AGAINST. Read these two CHANGELOG entries before deciding:

1. **v0.131.2** — `work-on-task-assistant` no longer spawns a second session onto a task the calling session is already working; the status flip stops routing through `vault-cli task work-on` when session-connect left `claude_session_id` empty, "which is the one case where `work-on` spawns instead of taking its cached path". That is precisely the empty-field window the reversion was created to close, addressed at the assistant layer instead of by pre-writing the field.

2. **v0.138.1** (2026-09-19) — the session-connect matcher was rebuilt: it now **strips** the leading decoration, **scopes** the scan to **live** transcripts (`LIVE_WINDOW` = 5 min, per `docs/session-liveness.md`), and **requires uniqueness**, refusing on zero or multiple matches. The reversion's root cause was an unfiltered mtime scan binding the most-recently-modified transcript; that scan no longer exists in that form. Measured in the entry: exact match 0 sessions vs decoration-stripped 1; across a real fleet 81 stripped titles collide, which is why uniqueness is also required.

Together these mean the reversion's stated failure mode is plausibly closed by later work — but "plausibly" is not "proven": neither entry was written as a spec-041 remediation, neither was verified against the post-exit ordering, and the empty-field window still exists between spawn and persist. The spec's Constraints section nevertheless requires this reorder ("No compensating clear ... delete them"), and AC7/AC9 pin it, so this prompt implements the spec AS APPROVED.

The reviewer must decide at audit time:
  (A) Approve — spec 041 wins; the reorder is re-applied. With v0.131.2 + v0.138.1 in place the session-connect mis-binding is much less likely, and the residual window is owned as a follow-up (e.g. a spec that gives the child a non-frontmatter source of truth for its session id).
...
4. **Reword the `workon.go` doc comments that describe the pre-spawn design.**
   ...
   - `sessionFailureResult`'s comment says the warnings are `the accumulated warnings (including any compensating-clear warning)`. Reword that clause — there is no clear, so the warnings are only the assignee and daily-note ones. Do not change the function's behaviour or its "deliberately not logged here" note.

5. **Confirm `goal_workon.go` AND `topic_workon.go` are already in the target state — do NOT change either.** Both are the structural templates the task path must match.
   ...
   - `topic_workon.go`: same shape — start-then-persist on both branches, `errors.Wrap(ctx, err, "start claude session")`, no compensating clear, cached path `return existing, nil`, and `persistTopicSessionID` returning an empty id on failure. No `clearTopicSession` may exist.
   This is the sibling-entry-point check: the repo has THREE work-on operations (task, goal, topic). Only the task path carries the reverted pre-spawn ordering; after this prompt all three must agree. If either sibling already matches, leave it untouched.
...
11. **Reword `pkg/ops/workon_session_writeback_test.go` to post-exit semantics — comments and titles ONLY (AC8).** The fakes already write a valid turn-JSON line to the `stdout *os.File` and exit cleanly via `done <- nil` with a blocking waiter; confirm that and do NOT change it. Every on-disk assertion in this file holds unchanged under the new ordering — the ids simply never land — so DO NOT touch any assertion. The full list of prose to change (this is every stale occurrence in the file; the drift-guard grep in `<verification>` requires all of them):
    - The task context's `BeforeEach` comment (`work-on persists the fresh session id and its metrics entry to the file BEFORE spawning (pre-spawn persist), then the spawned Claude session runs plan-task -> execute-task and writes its own frontmatter on top of that file inside the detached child (the detachRun fake). Nothing writes to the file after the child's own write, so the child's frontmatter survives.`) → `Simulate the real headless turn: work-on spawns the child first, then the Claude session runs plan-task -> execute-task and writes its own frontmatter on top of that file inside the detached child (the detachRun fake). Only after the child exits does work-on re-read and persist the session id, so the child's frontmatter survives.`
    ...
    - Reword the body comment that describes the pre-spawn persist + compensating clear re-read so it describes the post-exit, no-clear ordering: the turn failed, so nothing was ever written for this id, and the child's own `phase: planning` write is what survives.
    ...
    - Do NOT touch the pinned-count strings anywhere in this file: `TaskPhaseExecution`, `GoalPhaseExecution`, `session_note`, `MetricsSessions()`, `ClaudeSessionID()`. The AC8 greps must keep returning their exact counts (2, 2, 4, 2, 2), so your reword must not add or remove any occurrence of those tokens.

12. **Confirm the goal AC7 test and the goal assertions are already correct — do not touch them.** `goal_workon_test.go`'s `"when persisting the goal session id after the child exits"` already asserts `writeGoalAt.After(childExitAt)` with both non-zero, and its failed-turn context already proves the no-persist half. Leave the file unmodified.

13. **Confirm the spec's Failure Modes rows that this prompt owns.**
    - Row 6 (claude binary missing): the `ErrStarterUnavailable` soft path is unchanged — `handleClaudeSession` returns `"", ErrStarterUnavailable` when the starter is nil and `Execute` downgrades it to a warning. Confirm all three of `workon.go`, `goal_workon.go` and `topic_workon.go` still reference it.
...
- `goal_workon.go`, `goal_workon_test.go` and `topic_workon.go` are already in the target state — do not modify them except to read and confirm. The task path is the only one that needs the reorder; do not "harmonise" the siblings further.
...
grep -c 'ErrStarterUnavailable' pkg/ops/topic_workon.go                 # >= 1 (Failure Modes row 6)
! grep -q 'clearTopicSession' pkg/ops/topic_workon.go                   # sibling parity: no compensating clear
! grep -rq 'clearSessionAndMetrics' pkg/                              # AC9, repo-wide: no caller and no replacement
