...
<summary>
- Rewords the task-path sections of the work-on session lifecycle document so they describe both paths persisting the session id only after the headless turn exits, with no rollback step — removing the pre-spawn wording a later reversion left behind under an already-correct heading.
...
- Creates the changelog's unreleased section (it does not exist today — the newest section is a released version) and records the change under it with a conventional prefix.
...
- Replaces the spec's `git diff --exit-code HEAD` guard for the resume scenario with a content-hash check, because this container's `.git` is masked and every git command fails here.
</summary>
...
- `CHANGELOG.md` — read the top ~20 lines only (the frozen `# Changelog` preamble and the newest versioned section). That is where the new section and bullet land; the rest of the file is not needed.
- `scripts/check-changelog.sh` — the structural rule the changelog edit must satisfy (the preamble must precede every `## ` section).
...
**NOTE — git is NOT usable in this container** (`.git` is masked; every git command fails with `fatal: not a git repository`). The scenario-005 guard is therefore expressed as a content hash in prompt 1 and is not repeated here. Never treat a failed git command as a pass.
</context>
...
2. **Reword the stale bodies in `docs/work-on-session-lifecycle.md` to the post-exit, no-clear ordering.** The introduction (the paragraph mentioning "spec 040, revised by spec 041", "An id on disk now means the session is resumable") is already correct — keep it. Fix these four bodies, and nothing else:
   - **`## Post-exit write ordering`** — the heading is correct; the body is stale. Replace the whole body (the paragraph starting `On the **task path** the fresh id and its \`metrics_sessions\` entry are now persisted **before the child is spawned**...`, the paragraph starting `The **goal path** (\`pkg/ops/goal_workon.go\`) keeps its post-exit ordering unchanged: ...`, and the paragraph starting `On the task path the pre-spawn re-read before writing is load-bearing: ...`) with:
     ```
     ...
     ```
     The sentence about the goal path keeping its post-exit ordering unchanged is subsumed by the replacement — delete it rather than leaving it as a dangling contrast.
   - **`## What the turn timeout does and does not cover`** — one sentence is stale: `The compensating clear is unchanged: it still fires on every error the detached turn returns. Only the definition of "failed" moved.` Replace it with `Nothing is written before the turn, so no rollback step runs on any error the detached turn returns. Only the definition of "failed" moved.` Leave the rest of that section, including the "read is never hoisted above the select" paragraph, exactly as it is.
   - **`## Failure path`** — replace the body paragraph starting `On the **task path** the id is pre-persisted, so a failed spawn runs a compensating clear: ...` and ending `The **goal path** persists nothing on failure and needs no clear.` with:
     ```
   ...
   - **`## The per-session lock`** — the `**The detached-child safety property.**` paragraph is stale. Replace it with:
     ```
     **The detached-child safety property.** On the spawn path, when the parent stops
     ...
     ```
   - Do NOT touch `## Session id ownership`, `## Why stream-json was rejected`, `## Why the TTY branch is untouched`, `## The fate of --output-format json`, the rest of `## The per-session lock`, or the rest of `## What the turn timeout does and does not cover`. In particular, the phrase "liveness gating" in the lock's "Lock scope" paragraph is a spec-042 Vault UI follow-on concept, NOT the removed liveness-window concept — leave it. The phrase `no compensating clears` in the lock's "No stale lock" paragraph is about lock cleanup and is still true — leave it too; it is the ONE occurrence the drift-guard grep below tolerates. `scenarios/005-work-on-resume-auto-invokes-subtask.md` is never edited.
   - After the reword the file must contain none of the reverted vocabulary — see the drift-guard greps in `<verification>`.
...
4. **Create the unreleased section in `CHANGELOG.md` and record the change under it (AC12).** Today `CHANGELOG.md` has NO unreleased section — the newest section is `## v0.141.1`. Insert, immediately after the frozen preamble (the `* MAJOR version...` bullet block) and immediately above `## v0.141.1`:
   ```
   ## Unreleased
   ...
   - Do NOT bump the plugin version strings in `.claude-plugin/plugin.json` or `.claude-plugin/marketplace.json` (both currently `0.141.1`), and do NOT create a tag. `.maintainer.yaml` sets `release.autoRelease: true`, so this repository's release agent owns version bumps and tagging after merge (see `CLAUDE.md` § Plugin Release Checklist). Write `## Unreleased`, never `## vX.Y.Z`.
...
- The "liveness gating" phrase in the per-session lock section is spec 042's Vault UI follow-on concept — leave it; it is not the removed liveness-window concept. Likewise `no compensating clears` in the lock's "No stale lock" paragraph stays.
...
<verification>
Evidence greps — run each, record the count, and confirm it against the expectation. Rows expecting 0 are written as `! grep -q` because `grep -c` exits 1 when it prints 0. **Every pattern below is single-line-safe**: a phrase that the stale text wraps across two lines (e.g. `a failed spawn runs a compensating` / `clear: a re-read-modify-write`) is matched by its first line only, so the guard cannot silently pass because of a line break.

```
grep -c '^## Unreleased' CHANGELOG.md                                          # >= 1 (AC12) — currently 0, must flip to >= 1
grep -A15 '^## Unreleased' CHANGELOG.md | grep -ci 'resume'                    # >= 1 (AC12) — currently 0, must flip to >= 1
...
grep -c 'compensating clear' docs/work-on-session-lifecycle.md                 # == 1 (only the lock section's "no compensating clears"; currently 4)
...
! grep -q 'a failed spawn runs a compensating' docs/work-on-session-lifecycle.md   # currently matches (line 137) — the line-break-safe form
! grep -q 'The compensating clear is' docs/work-on-session-lifecycle.md            # currently matches (line 120) — the line-break-safe form
! grep -q 'the compensating clear removes the id' docs/work-on-session-lifecycle.md
! grep -q 'the fresh id and its `metrics_sessions` entry are now persisted' docs/work-on-session-lifecycle.md
! grep -q 'keeps its post-exit ordering unchanged' docs/work-on-session-lifecycle.md
...
grep -n -m1 '^## v0.141.1' CHANGELOG.md                                        # the last released section, must still exist
```
...
The spec's AC10 guard (`git diff --exit-code HEAD -- scenarios/005-work-on-resume-auto-invokes-subtask.md`) cannot run in this container (`.git` is masked) and was replaced by the content-hash check in prompt 1; it is not repeated here.
</verification>
