...
`createTempVaultWithBaseline` writes `Tasks/` task files, then a config of the shape `createTempVaultWithTopics` writes with one extra line — `    baseline: %s` — under the `test` vault, with `baselineFile` interpolated. When `baselineFile` is empty, omit the `baseline:` line entirely (that is the no-baseline fixture).
