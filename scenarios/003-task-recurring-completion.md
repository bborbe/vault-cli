...
- [ ] `grep -E "^defer_date: \"?$NEXT_WEEK\"?\$" "$TASK_FILE"` succeeds (deferred 1 week forward; YAML may quote the date)
- [ ] `grep -E "^last_completed: \"?$(date +%Y-%m-%d)\"?\$" "$TASK_FILE"` succeeds (last_completed stamped today)
