...
- [ ] **`StartSession` returns within `livenessWindow`** on a runner whose child outlives it — asserted against the constant with an injected clock, not a wall-clock tolerance. Evidence: `grep -c 'Expect(capturedWindow).To(Equal(ops.LivenessWindow))' pkg/ops/claude_session_test.go` returns 1, `grep -c 'Expect(capturedWindow).To(Equal(10 \* libtime.Second))' pkg/ops/claude_session_test.go` returns 1, and `grep -c 'time.Since(start)' pkg/ops/claude_session_test.go` returns 0.
...
- [ ] **The writeback invariant holds under a bounded diff.** `go test -v ./pkg/ops/ -ginkgo.focus="work-on session write-back"` exits 0 reporting `Ran N of …` with N ≥ 1, AND every invariant assertion survives the mechanical edit: in the post-fix file `grep -c 'TaskPhaseExecution'` returns 2, `grep -c 'GoalPhaseExecution'` returns 2, `grep -c 'session_note'` returns 4, `grep -c 'MetricsSessions()'` returns 2, **and `grep -c 'ClaudeSessionID()'` returns 2** — the id-persistence invariant is the one caller-side generation actually threatens, so it is locked explicitly. Counts are pinned to the current tree; the permitted edits (stub arity, pinned uuid constant) do not touch those lines.
